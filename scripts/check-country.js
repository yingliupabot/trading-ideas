...
(async function(){
  ...
  var geo=()=>pg.evaluate(()=>{
    ...
    return {view:TL.view(), 球:bx(".tl-globe-wrap"), 卷轴:bx(".tl-scroll"),
            ...
            铺开:!document.getElementById("tl-scroll").hasAttribute("hidden"),
  ...
  head("球是摊平的,不是切过去的");
  await pg.evaluate(()=>TL.closeCountry()); await pg.waitForTimeout(1400);
  var mo = await pg.evaluate(()=>new Promise(res=>{
    var out=[], t0=performance.now();
    TL.openCountry("United States of America");
    (function tick(){
      var cv=document.getElementById("tl-morph"), sv=document.getElementById("tl-cmap"),
          pp=document.getElementById("tl-scroll-paper");
      out.push({t:performance.now()-t0,
                画布:!cv.hasAttribute("hidden"),
                z:TL.globe.zoom,
                图:+getComputedStyle(sv).opacity, 纸:+getComputedStyle(pp).opacity});
      if (performance.now()-t0 < 2400) requestAnimationFrame(tick); else res(out);
    })();
  }));
  var onCanvas = mo.filter(function(r){ return r.画布; });
  pass &= ok(onCanvas.length >= 30, "摊平那一段由画布接管", onCanvas.length+" 帧");
  /* 交接不许有空窗:画布撤走时 SVG 必须已经是实的。
     反过来做会有十几帧空纸 —— 画布没了,SVG 还在淡入(实测踩过) */
  var blank = mo.filter(function(r){ return !r.画布 && (r.图 < 0.98 || r.纸 < 0.98); });
  pass &= ok(blank.length === 0, "画布交给 SVG 时不留空窗", blank.length+" 帧空窗");
  /* 推近要走过去,不能一帧跳到位 */
  var zs = {}; mo.forEach(function(r){ if (r.t < 600) zs[r.z.toFixed(2)] = 1; });
  pass &= ok(Object.keys(zs).length >= 8, "推近是连续的,不是一帧跳过去", Object.keys(zs).length+" 个中间倍率");

  head("镜头是连着的:先推到国家跟前,再铺纸");
  await pg.evaluate(()=>TL.closeCountry()); await pg.waitForTimeout(1900);
  await pg.evaluate(()=>TL.openCountry("United Kingdom"));
  ...
  head("小球是回程的按钮");
  await pg.evaluate(()=>TL.openCountry("Japan")); await pg.waitForTimeout(2200);
  var box = await pg.evaluate(()=>{var r=document.getElementById("world-map").getBoundingClientRect();
    return {x:r.left+r.width/2, y:r.top+r.height/2};});
  await pg.mouse.click(box.x, box.y); await pg.waitForTimeout(1900);
  pass &= ok((await geo()).view==="globe", "点小球退回全局视图");

  head("Esc 也能退");
  await pg.evaluate(()=>TL.openCountry("China")); await pg.waitForTimeout(2200);
  await pg.keyboard.press("Escape"); await pg.waitForTimeout(1900);
  pass &= ok((await geo()).view==="globe", "Esc 退回全局视图");

  head("没内容的国家");
  await pg.evaluate(()=>TL.openCountry("Brazil")); await pg.waitForTimeout(2200);
  ...
  for (var n of ["France","Japan","United Kingdom","Germany"]) {
    await pg.evaluate(k=>TL.openCountry(k), n); await pg.waitForTimeout(900);
    await pg.evaluate(()=>TL.closeCountry()); await pg.waitForTimeout(500);
  }
  await pg.evaluate(()=>TL.openCountry("United Kingdom")); await pg.waitForTimeout(2200);
